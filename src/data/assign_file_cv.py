        ...
        for category_i in ["train", "val", "test"]:
            for folder_name_i in ["waste", "non_waste"]:
                if category_i in ["train", "val"]:
                    ...
                    folder_path = folder_base_path + "images_test" + "/cv" + str(cv_i) + "/" + category_i + "/" + folder_name_i
...
def generate_tiles_for_cv(df, cv_i, layer_i, seed=111):
    ...
    # ---- Validation set: 10% sampled within each fold (except cv_i) ----
    df_not_test = df_based[df_based["fold"] != cv_i]

    # Equivalent of dplyr::slice_sample(prop=0.15, by="fold")
    # 12.5 means that 10% of full samples is 12.5% of remaining 80%
    np.random.seed(seed)
    df_cv_val = (
...
def generate_cv_files(df, layer_i):
    list_cv1 = generate_tiles_for_cv(df, cv_i=0, layer_i=layer_i)
    list_cv2 = generate_tiles_for_cv(df, cv_i=1, layer_i=layer_i)
    list_cv3 = generate_tiles_for_cv(df, cv_i=2, layer_i=layer_i)
    list_cv4 = generate_tiles_for_cv(df, cv_i=3, layer_i=layer_i)
    list_cv5 = generate_tiles_for_cv(df, cv_i=4, layer_i=layer_i)
...
def assign_files(list_cv, layer_i: str, cv_i = None):
    """
        ...
        for category_i in ["train", "val", "test"]:
            ...
            for fname in list_files:
                ...
                if category_i in ["train", "val"]:
                    ...
                    dst = folder_base_path + "images_test" + "/cv" + str(cv_i) + "/" + category_i + "/" + layer_i + "/" + fname + ".tif"
